...
pub fn derive_preset_id(podcast_guid: Option<&str>, feed_urls: &[String]) -> Option<String> {
...
mod tests {
    ...
    #[test]
    fn derive_from_guid() {
        let id = derive_preset_id(Some("abcdef-1234"), &[]).unwrap();
        assert_eq!(id.len(), 12);
        assert!(id.chars().all(|c| c.is_ascii_hexdigit()));
        let id2 = derive_preset_id(Some("abcdef-1234"), &[]).unwrap();
        assert_eq!(id, id2);
    }
    ...
    fn derive_from_feed_url_when_no_guid() {
        ...
        let id = derive_preset_id(None, &urls).unwrap();
        assert_eq!(id.len(), 12);
        assert!(id.chars().all(|c| c.is_ascii_hexdigit()));
    }
    ...
    fn guid_takes_priority_over_feed_url() {
        let urls = vec!["https://example.com/feed.xml".to_string()];
        let from_guid = derive_preset_id(Some("my-guid"), &urls).unwrap();
        let from_url = derive_preset_id(None, &urls).unwrap();
    ...
    fn empty_guid_falls_through_to_feed_url() {
        let urls = vec!["https://example.com/feed.xml".to_string()];
        let from_empty_guid = derive_preset_id(Some(""), &urls).unwrap();
        let from_none_guid = derive_preset_id(None, &urls).unwrap();
        assert_eq!(from_empty_guid, from_none_guid);
    }

    #[test]
    fn returns_none_when_no_input() {
        assert!(derive_preset_id(None, &[]).is_none());
        assert!(derive_preset_id(Some(""), &[]).is_none());
    }
    ...
    fn skips_empty_feed_urls() {
        ...
        let id = derive_preset_id(None, &urls).unwrap();
        let expected =
            derive_preset_id(None, &["https://example.com/feed.xml".to_string()]).unwrap();
        assert_eq!(id, expected);
    }

    #[test]
    fn returns_none_when_all_feed_urls_empty() {
        assert!(derive_preset_id(None, &["".to_string()]).is_none());
        assert!(derive_preset_id(Some(""), &["".to_string()]).is_none());
    }
    ...
    fn trims_whitespace_from_guid() {
        let trimmed = derive_preset_id(Some("abcdef-1234"), &[]).unwrap();
        let padded = derive_preset_id(Some("  abcdef-1234  "), &[]).unwrap();
    ...
    fn trims_whitespace_from_feed_urls() {
        ...
        let trimmed = derive_preset_id(None, &trimmed_urls).unwrap();
        let padded = derive_preset_id(None, &padded_urls).unwrap();
        assert_eq!(trimmed, padded);
    }
    ...
    fn whitespace_only_guid_falls_through() {
        let urls = vec!["https://example.com/feed.xml".to_string()];
        let from_ws_guid = derive_preset_id(Some("   "), &urls).unwrap();
        let from_none_guid = derive_preset_id(None, &urls).unwrap();
    ...
    fn known_md5_vector() {
        ...
        let id = derive_preset_id(Some("hello"), &[]).unwrap();
